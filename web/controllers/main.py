...
import operator
import os
#import stat # verifica modo de permiso de archivos 
import re
import sys
import tempfile
import unicodedata
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['pattern'] = DBNAME_PATTERN # Nombre de la Base de Datos        
...
class ExportFormat(object):
    ...
    def filename(self, base): # Nombre del Archivo sin la Extension (Nombre de la Base de Datos)
        ...
        if model_description == 'Pay Slip':
            model_description = 'Nomina UPTYAB ' #+fecha1
            
        return f"{model_description} ({base})"

    def from_data(self, fields, row):
...
class TXTExport(ExportFormat,http.Controller):
    ...
    @property
    def content_type(self):
        return 'text/txt;charset=utf8'

    @property
    def extension(self):
        return '.txt'

    def from_group_data(self,groups,fields):
        raise UserError(_("Exportar Datos por Grupos no esta Soportado....."))
    
    def from_data(self, fields, rows):
        #campos="".join(fields) #pasa a formato de cadena los datos de lista de fields  
             
        fp = io.StringIO()       
        # * * * * * * * * * * * * * * * * * * * * *
        # cantidad de datos a procesar
        cant=len(rows) # longitud de filas
        ...
        n_trab=str(n_trab) 
        lista=str(L) 
        quitar=" ,'[]" #elementos o caracteres no deseados de la lista
        lista= lista.translate(str.maketrans('', '', quitar)) #quita elementos de la lista      
            
        freq = lista.count(n_trab)
        
        oper = list(dict.fromkeys(lista)) # Deja valores unicos de numero de trabajador
        oper=str(oper)
        oper= oper.translate(str.maketrans('', '', quitar)) #quita elemntos de la lista   
        oper=len(oper)       
        freq=str(freq)
           
        oper=str(oper).zfill(7)
        total="{:.2f}".format(total)
        total=str(total)
        total=total.zfill(15).replace(".","")      
        
        #fp.write(lista+'\n') 
        fp.write("ONTNOM"+rif+oper+total+'VES'+fecha+'\n')             
        ...
        for y in range(cant):
            ...
            if nombre!="": # Nombre del Trabajador     
                n_trab +=1 
                n_trab=str(n_trab) 
                freq = lista.count(n_trab) # la cantidad de montos por trabajador
                veces=freq
               
             
            for x in [-7,-6,-5,-4,-3,-2,-1]: # rango de datos en x
                dato=str(rows[y][x])                    
                freq=int(freq)
                yy=y-freq+1 # Cursor
                ...
                if x==-5:
                        n_trab=int(n_trab)
                        dato=str(rows[y][x])
                        if dato=="": # Empleado sin monto a ser pagado (No se imprimira)
                            dato=0                   
                        ...
                        if veces==0:             
                            if freq==1: # En el caso que solo el trabajador tenga un solo monto
                                nacionalidad=str(rows[y][-2])
                                ...
                                cedula=str(rows[y][-3]).zfill(8)   
                                cuenta=str(rows[y][-4]).zfill(20) 
                                subtotal=str(subtotal).zfill(12).replace(".","")                               
                                nombre=str(rows[y][-6]).replace("Á","A").replace("É","E").replace("Í","I")\
                                    .replace("Ó","O").replace("Ú","U").replace("À","A").replace("È","E")\
                                    .replace("Ì","I").replace("Ò","O").replace("Ù","U").replace("á","a")\
                                    .replace("é","e").replace("í","i").replace("ó","o").replace("ú","u")\
                                    .replace("à","a").replace("è","e").replace("ì","i").replace("ò","o")\
                                    .replace("ù","u").replace("Ü","U").replace("ü","u").replace("Ñ","N")\
                                    .replace("ñ","n").replace("`"," ").replace("´"," ")
                                 
                                fp.write(nac+cedula+cuenta+subtotal+nombre+'\n')                               
                                subtotal=0 # reset monto para nuevo trabajador   
                                              
                            else: # en el caso que el trabajador tenga mas de un monto
                                nacionalidad=str(rows[yy][-2])
                                nacionalidad=nacionalidad.upper()
                                if nacionalidad=="VENEZUELA":
                                    nac="V"
                                else:
                                ...
                                cedula=str(rows[yy][-3]).zfill(8) 
                                cuenta=str(rows[yy][-4]).zfill(20)
                                subtotal=str(subtotal).zfill(12).replace(".","")                               
                                nombre=str(rows[yy][-6]).replace("Á","A").replace("É","E").replace("Í","I")\
                                    .replace("Ó","O").replace("Ú","U").replace("À","A").replace("È","E")\
                                    .replace("Ì","I").replace("Ò","O").replace("Ù","U").replace("á","a")\
                                    .replace("é","e").replace("í","i").replace("ó","o").replace("ú","u")\
                                    .replace("à","a").replace("è","e").replace("ì","i").replace("ò","o")\
