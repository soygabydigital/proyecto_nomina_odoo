...
from collections import Counter # modificado 
    ...
    def from_data(self, fields, rows):
        ...
        for y in range(cant):
            ...
            for x in [-6,-5,-4,-3,-2]:
                ...
                if x==-5: 
                    if dato=="":
                        dato=str(rows[yy][-5]).zfill(8)
                    else:   
                        cedula=dato.zfill(8)
                
                if x==-4:
                    if dato=="": 
                        dato=str(rows[yy][-4]).zfill(20)
                    else:
                        cuenta=dato.zfill(20)
                ...
                #n_trab=int(n_trab)
                    dato=str(rows[y][-3])
                    if dato=="": # Empleado sin monto a ser pagado
                        monto="0"                    
                        ...
                        #if veces==0 and freq==1:
                        #monto=dato
                        """n_trab=str(n_trab)
                        operaciones=str(operaciones)
                        freq=str(freq)
                        monto=str(monto).zfill(12)
                        veces=str(veces)                          
                        _guardar(nac,cedula,cuenta,monto,nombre)
                        monto=0"""
                        #if veces>0:
                        dato=float(dato)
                        monto=float(monto) 
            ...
            veces=str(veces)     
            #if veces=="0": 
            fp.write(n_trab+" "+freq+" "+veces+" "+nac+" "+cedula+" "+cuenta+" "+monto+" "+nombre)
            fp.write('\n')"""           
                
            def _guardar(nac,cedula,cuenta,monto,nombre):
